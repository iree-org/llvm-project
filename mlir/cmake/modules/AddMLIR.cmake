...
function(add_mlir_aggregate name)
  ...
  # On Windows with MSVC, convert -force_load flags to /WHOLEARCHIVE
  # Must use LINKER: prefix or shell: prefix to prevent Ninja from treating as file dependency
  if(WIN32 AND MSVC)
    get_target_property(_link_libs ${name} LINK_LIBRARIES)
    if(_link_libs)
      set(_new_link_libs "")
      set(_next_is_lib FALSE)

      foreach(_item ${_link_libs})
        if("${_item}" STREQUAL "-force_load")
          set(_next_is_lib TRUE)
        elseif(_next_is_lib)
          # Remove from LINK_LIBRARIES and add via link options with LINKER: prefix
          # Using SHELL: to pass the argument as-is without CMake parsing it
          target_link_options(${name} PRIVATE "SHELL:/WHOLEARCHIVE:${_item}")
          set(_next_is_lib FALSE)
        else()
          list(APPEND _new_link_libs "${_item}")
        endif()
      endforeach()

      set_target_properties(${name} PROPERTIES LINK_LIBRARIES "${_new_link_libs}")
    endif()
  endif()
