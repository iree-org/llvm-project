...
public:
  ...
  FailureOr<Operation *> generate(ArrayRef<int64_t> inputVecSizes = {},
    ...
    // When both inputChannel and outputChannel have been folded away (e.g. a
    // convolution with C_in=C_out=1 after unit-dim folding), the structural
    // classification marks the op as "pooling" and shapes the filter as 1-D
    // {kw}.  If the body is actually a convolution (multiply + add), the
    // contraction path would receive a scalar filter slice instead of the
    // expected vector<c, f>, producing invalid IR.  Bail out.
    if (config.isPooling && convKind != ConvOperationKind::Pool)
      return failure();
